...
def hough(edge, ThetaDim, DistStep, threshold=None):
    max_dis = numpy.sqrt(edge.shape[0]**2 + edge.shape[1]**2)
    ...
    for i in range(result.shape[1]):
        eight_neiborhood = accumulator[max(0, result[0, i] - 3):min(result[0, i] + 2, accumulator.shape[0]), max(
            0, result[1, i] - half_dist_window_size + 1):min(result[1, i] + half_dist_window_size, accumulator.shape[1])]
...
def draw_lines(lines, blurred, edge, color=(255, 165, 0), err=3):
    result = blurred
...
if __name__ == '__main__':
    img = plt.imread('imageWork2/IMG_20200401_171217.jpg')
    blurred = cv2.GaussianBlur(img, (3, 3), 0)
    edge = cv2.Canny(blurred, 50, 150)
    # edge = canny.canny(blurred, 50, 100)
    plt.imshow(edge, cmap='gray')
    plt.show()
    lines = hough(edge, 90, 1)
    final_img = draw_lines(lines, blurred, edge)
